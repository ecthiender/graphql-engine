...
getFldInfo
  :: (MonadError (QErr code) m, AsCodeHasura code, MonadReader r m, Has FieldMap r)
  => G.NamedType -> G.Name
  -> m ResolveField
getFldInfo nt n = do
  fldMap <- asks getter
...
fieldAsPath :: (MonadError (QErr code) m) => Field -> m a -> m a
fieldAsPath = nameAsPath . _fName

resolvePGCol :: (MonadError (QErr code) m, AsCodeHasura code)
