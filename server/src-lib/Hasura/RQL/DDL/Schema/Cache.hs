...
mergeCustomTypes
  :: (MonadError (QErr code) f, AsCodeHasura code)
...
buildRebuildableSchemaCache
  :: (HasVersion, MonadIO m, MonadUnique m, MonadTx code m, HasHttpManager m, HasSQLGenCtx m, AsCodeHasura code)
...
newtype CacheRWT m a
  ...
  deriving
    ( Functor, Applicative, Monad, MonadIO, MonadReader r, MonadError e, MonadTx code
...
instance (MonadIO m, MonadTx code m, AsCodeHasura code) => CacheRWM (CacheRWT m) where
...
buildSchemaCacheRule
  ...
  :: ( HasVersion, ArrowChoice arr, Inc.ArrowDistribute arr, Inc.ArrowCache m arr
     , MonadIO m, MonadTx code m, MonadReader BuildReason m, HasHttpManager m, HasSQLGenCtx m, AsCodeHasura code )
  ...
  where
    buildAndCollectInfo
      :: ( ArrowChoice arr, Inc.ArrowDistribute arr, Inc.ArrowCache m arr
         ...
         , HasHttpManager m, HasSQLGenCtx m, AsCodeHasura code )
    ...
    buildTableEventTriggers
      :: ( ArrowChoice arr, Inc.ArrowDistribute arr, ArrowWriter (Seq CollectedInfo) arr
         , Inc.ArrowCache m arr, MonadIO m, MonadTx code m, MonadReader BuildReason m, HasSQLGenCtx m, AsCodeHasura code )
      ...
      where
        ...
        buildRemoteSchema = Inc.cache proc (invalidationKeys, remoteSchema) -> do
          ...
          (| withRecordInconsistency (liftEitherA <<< bindA -<
               ...
               runExceptT $ undefined) --addRemoteSchemaP2Setup remoteSchema)
           |) (mkRemoteSchemaMetadataObject remoteSchema)
...
withMetadataCheck :: (MonadTx code m, AsCodeHasura code, CacheRWM m, HasSQLGenCtx m) => Bool -> m a -> m a
withMetadataCheck cascade action = do
  ...
  unless (null overloadedFuncs) $
    throw400 (_NotSupported # ()) $ "the following tracked function(s) cannot be overloaded: "
  ...
  where
    ...
    processSchemaChanges :: (MonadTx code m, AsCodeHasura code, CacheRM m) => SchemaDiff -> m ()
